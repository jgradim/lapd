...
With the incredible advances on Javascript Engines rendering speeds and the extensive use of AJAX by web applications, JSON is becoming the prefered API output language by developers both because it can be parsed by Javascript's eval function.

% NAO SEI ATE QUE PONTO EXPRESSAR 1a OPINIÃO É CORRECTO
Unfortunately some of the most used API's only provide an XML dialect output, so the objective of this project is to create a web application that acts as a proxy for any API, transforming the XML output into JSON.
...
The next step will be to fully document all the idiosyncrasies of the XML translation into JSON and turn the statistics module into a monitoring module in order to get more data in bugs that might arise with the addition of newer API\'s.
